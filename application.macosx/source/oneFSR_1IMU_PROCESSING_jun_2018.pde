...
import themidibus.*; //MIDI IO Library
import g4p_controls.*;
int fsr1 = 0;
int fsr2 = 0;
int FSR1MIN = 100;
  ...
  if (myPort.available() >= 18) {
    ...
    if (inputString != null && inputString.length() > 0) {
      ...
      if (inputStringArr.length >= 5) { // q1,q2,q3,q4,\r\n so we have 5 elements
        ...
        fsr1 = int(inputStringArr[4]);
        fsr2 = int(inputStringArr[5]);
...
void ROLL(int theValue) {

  // myBus.sendControllerChange(channel, number, value); //SYNTAX
  int midivalue = int(map(theValue, ROLLMIN, ROLLMAX, 0, 127));
  myBus.sendControllerChange(0, 1, midivalue); // Send a controllerChange
}


void PITCH(int theValue) {

  // myBus.sendControllerChange(channel, number, value); //SYNTAX
  int midivalue = int(map(theValue, PITCHMIN,PITCHMAX, 0, 127));
  myBus.sendControllerChange(0, 2, midivalue); // Send a controllerChange
}



void YAW(int theValue) {

  // myBus.sendControllerChange(channel, number, value); //SYNTAX
  int midivalue = int(map(theValue, YAWMIN, YAWMAX, 0, 127));
  myBus.sendControllerChange(0, 3, midivalue); // Send a controllerChange
}



void FSR1(int theValue) {

  // myBus.sendControllerChange(channel, number, value); //SYNTAX
  int midivalue = int(map(theValue, FSR1MIN, 700, 0, 127));
  myBus.sendControllerChange(0, 4, midivalue); // Send a controllerChange
     
}
void FSR1MIN(int theValue) {
FSR1MIN = theValue;
}
void FSR2(int theValue) {

  // myBus.sendControllerChange(channel, number, value); //SYNTAX
  int midivalue = int(map(theValue, FSR2MIN, 933, 0, 127));
...
void ROLLMIN(int theValue) {

  // myBus.sendControllerChange(channel, number, value); //SYNTAX
ROLLMIN = theValue;
}
void ROLLMAX(int theValue) {

  // myBus.sendControllerChange(channel, number, value); //SYNTAX
ROLLMAX = theValue;
}

void YAWMIN(int theValue) {

  // myBus.sendControllerChange(channel, number, value); //SYNTAX
YAWMIN = theValue;
}

void YAWMAX(int theValue) {
